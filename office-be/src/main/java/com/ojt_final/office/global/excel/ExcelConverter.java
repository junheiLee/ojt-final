...
public class ExcelConverter {
    ...
    public boolean supports(String fileName) {
        ...
        return Arrays.asList(ExcelConst.EXCEL_EXTENSIONS).contains(extension);
    }
    ...
    public <T extends ExcelProcessable> List<T> read(InputStream inputStream,
        ...
        Sheet sheet = wb.getSheetAt(ExcelConst.START_INDEX); // 엑셀 파일의 첫 번째 sheet

        // 첫 행의 헤더로 DB 저장 가능 여부 확인
        validateHeaderRows(sheet.getRow(ExcelConst.START_INDEX), getHeaderNames(ExcelConst.Act.UPLOAD, targetDomain));
    ...
    private <T extends ExcelProcessable> List<T> readDataRows(Sheet sheet,
        ...
        for (Row row : sheet) {
            ...
            if (row.getRowNum() == ExcelConst.START_INDEX) { // skip header row
                continue;
            }
    ...
    private <T extends ExcelProcessable> T readRow(Row row, Class<T> domain) {
        ...
        try {
            ...
            domain.getMethod(ExcelConst.SET_VALUES_FROM_EXCEL, Row.class).invoke(item, row);
            return item;
    ...
    public <T extends ExcelProcessable> byte[] write(List<T> items,
        ...
        // 첫 행 헤더 정보 기입
        writeHeaderRows(sheet.createRow(ExcelConst.START_INDEX), getHeaderNames(ExcelConst.Act.DOWNLOAD, targetDomain));
    ...
    private void writeHeaderRows(Row headerRow, List<String> fieldNames) {
        ...
        int cellIdx = ExcelConst.START_INDEX;
    ...
    private <T> void writeDataRows(Sheet sheet, List<T> items, Class<T> targetDomain) {
        int rowIdx = ExcelConst.START_INDEX;
    ...
    private <T> void writeCell(Row row, int cellIdx, T item, Field field) {
        ...
        try {
            switch (type) {
                case ExcelConst.INT -> row.createCell(cellIdx, CellType.NUMERIC).setCellValue((double) field.get(item));
                case ExcelConst.DATE -> row.createCell(cellIdx, CellType.FORMULA).setCellValue((Date) field.get(item));
    ...
    private <T> List<String> getHeaderNames(ExcelConst.Act action, Class<T> domain) {
